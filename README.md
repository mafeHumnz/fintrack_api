...
Swagger generation is configured in `src/config/swagger.ts` with `swagger-jsdoc`, and the generated OpenAPI spec is ready to be exposed. I verified the current `src/app.ts` file and there is no Swagger UI mount in place yet; there are no `swagger-ui-express` or `swaggerUi.setup(...)` calls in the actual app code right now.

Current status:
- OpenAPI generation: configured
- Swagger UI: not mounted in the current `src/app.ts`
- Route that would expose the UI once enabled: `/api-docs`

If you want to enable the docs in the app, the required pattern is:
...
No explicit license file was found in the repository at the moment of review, so this is a placeholder recommendation:

- License: MIT
- Status: to be confirmed by the project owner

If you do not have a legal preference yet, MIT is the simplest default for a portfolio project, but it should be confirmed before publishing.
...
If you want, I can now turn this draft into the actual file contents and save it to `README.md`, and I can also create the root `.env.example` file with the exact placeholder values.
